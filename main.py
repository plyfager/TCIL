...
def test(_run, _rnd, _seed):
    ...
    for taski in range(inc_dataset.n_tasks):
        ...
        ypred, _ = model.eval_task(test_loader)
        ypred = ypred.argmax(1)

        with open(f"/mnt/yangyifei/codes/cs701/results/result_{taski+1}.txt", "w") as f:
            for i, res in enumerate(ypred):
                if i<789:
                    f.write(f"{str(i).zfill(3)}.jpg {res}")
                else:
                    f.write(f"{str(i+1).zfill(3)}.jpg {res}")
                f.write("\n")
        # ypred to txt 
